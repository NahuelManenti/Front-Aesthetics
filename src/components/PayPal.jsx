...
export default function PayPal() {
    const basket = useSelector((store) => store.checkoutReducer.carrito)
    const total = useSelector((store) => store.checkoutReducer.total)
    ...
    const initialOptions = { // Genero las opciones para enviarle al CDN
        "client-id": "AUGfmOex2ofLmGn8JSAkcaaQWEBsY6tXVn_MKThvVEvzw0vqKC-88UVgP5vyOzr7NiwP-JhGnFSkYcDq",
    ...
    const onApprove = (data, actions) => { //recibo el resultado de mi operacion
        ...
        return actions.order.capture()
            .then(function (details) {
                ...
                swal('Transaction ' + transaction.status + ': ' + transaction.id + '\n\nPor favor, verifica tu email para ver los detalles de la compra.');
                if (transaction.status == 'COMPLETED') {
                    dispatch(checkoutActions.cleanCarrito())
                }
                // alert('Transaction '+ transaction.status + ': ' + transaction.id + '\n\nSee console for all available details');
                // console.log(details.purchase_units[0].shipping)
                // console.log(basket)

                let dataCheckout = details.purchase_units[0].shipping
                console.log(dataCheckout)

                // dispatch(checkoutActions.sendInfo(dataCheckout, basket))
